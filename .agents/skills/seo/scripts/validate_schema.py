...
def validate_jsonld(content: str) -> List[str]:
    ...
    for i, block in enumerate(blocks, 1):
        ...
        if isinstance(data, list):
            ...
            if "@graph" in data:
                ctx = data.get("@context", "https://schema.org")
...
def _validate_schema_object(obj: dict, block_num: int) -> List[str]:
    ...
    types_to_check = raw_types if isinstance(raw_types, list) else [raw_types]
    deprecated = {
    ...
    for stype in types_to_check:
        if stype in deprecated:
            errors.append(f"{prefix}: @type '{stype}' is {deprecated[stype]}")
        if stype in restricted:
            # Healthcare sites qualify for FAQPage
            pass
